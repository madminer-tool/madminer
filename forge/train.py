...
from forge.ml.utils import create_missing_folders, load_and_check
...
def train(model_filename,
    ...
    logging.info('  Model saved at:         %s', model_filename)
    ...
    # Check paths
    create_missing_folders([os.path.dirname(model_filename)])
    ...
    # Save models
    logging.info('Saving learned model to %s', model_filename)
    model.save_state_dict(model_filename)
