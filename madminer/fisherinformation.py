...
class InformationGeometry():
    ...
    * `InformationGeometry.information_from_grid()` loads a grid of Fisher Informations
    ...
    def _information(self,theta):
        ...
        #check input format
        assert len(theta) == self.dimension, \
            "theta should have length %r, not %r" % (self.dimension,len(theta))
        ...
        #check output format
        assert np.shape(information)==(self.dimension,self.dimension), \
            "information should have shape %r, not %r" % ((self.dimension,self.dimension),np.shape(information))
    ...
    def _information_derivative(self,theta):
        ...
        return np.array([(self._information(theta+dtheta[k])-self._information(theta)) / epsilon for k in range(len(theta))])
    ...
    def find_trajectory(self,theta0,dtheta0,limits,stepsize=1):
        ...
        while in_grid and counter<200:
            ...
            ddtheta = -1.* np.einsum("abc,b,c->a", self._christoffel(theta), dtheta, dtheta)
            ...
            if np.isnan(dist):  break
            output_theta.append(theta*1.)
            output_dist.append(dist*1.)

            #check if outside range
            for th,lim in zip(theta,limits):
                if (th<lim[0] or th>lim[1]): in_grid=False
    ...
    def distance_contours(
        ...
        if stepsize==None:
            stepsize = min([(limit[1]-limit[0])/20. for limit in grid_ranges])
        if ntrajectories==None:
            ntrajectories = 20*self.dimension
        if self.dimension is not 2:
            continous_sampling = False
        limits=(1.+2.*stepsize)*np.array(grid_ranges)
        ...
        for i in range(ntrajectories):
            if continous_sampling:
                angle = 2.*np.pi/float(ntrajectories)*i
