...
class SimpleMorpher:
    """
    Simple morphing functionality. Morphing is a technique that allows MadMax to infer the full probability distribution
    ...
    (e.g. to optimize the morphing basis), the SimpleMorpher and AdvancedMorpher classes may be relevant. The difference
    between them is that `SimpleMorpher` assumes just one configuration of parameters, i.e. maximal powers of parameters
    contributing to the squared matrix element. In contrast, the `AdvancedMorpher` class allows for the definition of
    multiple such configurations, which can be helpful when there are different classes of processes contributing to
    ...
    * `calculate_morphing_weights()` calculates the morphing weights `w_c(theta)` for a given parameter point `theta`
    such that `p(theta) = sum_c w_c(theta) p(theta_c)`.

    * `calculate_morphing_weight_gradient()` calculates the gradient of the morphing weights, `grad_theta w_c(theta)`.

    Parameters
    ----------
    parameters_from_madminer : OrderedDict or None
    ...
    parameter_max_power : None or list of int
    ...
    parameter_range : None or list of tuple of float
    ...
    def __init__(self,
        ...
        # Initialize components, basis, morphing matrix
    ...
    def find_components(self,
        ...
        max_overall_power : int, optional
             The maximal sum of powers of all parameters contributing to the squared matrix element. If a tuple is
             given, gives the maximal sum of powers for each of several operator configurations (see `add_parameter`).
...
class AdvancedMorpher:
    """
    Advanced morphing functionality. Morphing is a technique that allows MadMax to infer the full probability
    distribution `p(x_i | theta)` for each simulated event `x_i` and any `theta`, not just the benchmarks.
