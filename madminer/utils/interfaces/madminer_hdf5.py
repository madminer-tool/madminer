...
def madminer_event_loader(
    ...
    with h5py.File(filename, "r") as f:
        ...
        try:
            ...
            sampling_ids = None
            logger.debug("sampling_benchmarks field not found, probably due to the data being generated by an older"
                         " MadMiner version")
...
def save_events_to_madminer_file(
    ...
    with h5py.File(filename, io_tag) as f:
        ...
        if events_per_sampling_benchmark is not None:
            f.create_dataset("sample_summary/events_per_sampling_benchmark", data=np.array(events_per_sampling_benchmark, dtype=np.int))
