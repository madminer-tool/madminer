...
def prepare_run_mg_pythia(
    ...
    mg_directory_placeholder = r"${1: -" + mg_directory + "}"
    mg_process_directory_placeholder = r"${2: -" + mg_process_directory + "}"

    # Find filenames for process card and script
    if proc_card_filename is None:
        for i in range(1000):
            proc_card_filename = mg_process_directory_placeholder + "/Cards/start_event_generation_{}.mg5".format(i)
    ...
    script_file_placeholder = mg_process_directory_placeholder + "/madminer/scripts/" + script_filename
    ...
    if run_card_file is not None:
        copy_commands += "cp {} {}\n".format(run_card_file, mg_process_directory_placeholder + "/Cards/run_card.dat")
    if param_card_file is not None:
        copy_commands += "cp {} {}\n".format(
            param_card_file, mg_process_directory_placeholder + "/Cards/param_card.dat"
        )
    if reweight_card_file is not None and not is_background:
        copy_commands += "cp {} {}\n".format(
            reweight_card_file, mg_process_directory_placeholder + "/Cards/reweight_card.dat"
        )
    if pythia8_card_file is not None:
        copy_commands += "cp {} {}\n".format(
            pythia8_card_file, mg_process_directory_placeholder + "/Cards/pythia8_card.dat"
        )

    # Put together script
    script = (
        "#!/bin/bash\n\n# Script generated by MadMiner\n\n# Usage: {} [MG_directory] [MG_process_directory]\n\n"
        + "{}\n\n{}\n{}/bin/mg5_aMC {} > {}\n"
    ).format(script_filename, initial_command, copy_commands, mg_directory_placeholder, proc_card_filename, log_file)
    ...
    call_placeholder = "{} {} {}".format(
        script_file_placeholder, mg_directory_placeholder, mg_process_directory_placeholder
    )
